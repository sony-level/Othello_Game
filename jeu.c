...
{
    int fenetreLargeur = ecran->w;
    int fenetreHauteur = ecran->h;
    ...
    SDL_Surface* texte = TTF_RenderText_Solid(police, "Bravo !! Le joueur Blanc remporte la partie.", couleurRouge);
    //SDL_Surface* textes = TTF_RenderText_Solid(police, "Appuyez sur << ECHAP >> pour Quitter.", couleurRouge);
    int texteLargeur = texte->w;
    int texteHauteur = texte->h;
    int textesLargeur = textes->w;
    int textesHauteur = textes->h;
    SDL_Rect position;
    position.x = (fenetreLargeur - texteLargeur ) / 2; // Centrer horizontalement
    position.y = (fenetreHauteur - texteHauteur) / 2; // Centrer verticalement
        SDL_Rect positions;
    positions.x = (fenetreLargeur - textesLargeur) / 2; // Centrer horizontalement
    positions.y = (fenetreHauteur - textesHauteur) / 2; // Centrer verticalement
    SDL_BlitSurface(texte, NULL, ecran, &position +3);
    SDL_BlitSurface(textes, NULL, ecran, &positions);
    ...
    police=TTF_OpenFont("police.ttf",30);
    SDL_Color couleurRouge = {255, 0, 0};
    SDL_Surface* texte = TTF_RenderText_Solid(police, "Bravo !! Le joueur Noir remporte la partie.", couleurRouge);
    //SDL_Surface* textes = TTF_RenderText_Solid(police, "Appuyez sur << ECHAP >> pour Quitter.", couleurRouge);
    int texteLargeur = texte->w;
    int texteHauteur = texte->h;
    int textesLargeur = textes->w;
